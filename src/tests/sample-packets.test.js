...
describe('sample packet library', () => {
  it('contains several documented packets with expected files and images', () => {
    expect(manifest.packets.length).toBeGreaterThanOrEqual(6);
  ...
  it('keeps the tequila-derived synthetic packet passing', () => {
    const packet = manifest.packets.find((item) => item.id === 'tequila-extracted-synthetic');
    const expected = readJson(packet.expectedPath);
    const fixture = readJson(packet.ocrFixturePath);
    const files = packet.images.map((image) => ({
      name: image.name,
    ...
    expect(review.fields.find((field) => field.field === 'Brand Name')?.status).toBe('PASS');
    expect(review.fields.find((field) => field.field === 'Government Warning')?.status).toBe('PASS');
  });
});
