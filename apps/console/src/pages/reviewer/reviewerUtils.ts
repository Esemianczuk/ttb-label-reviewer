...
export function unresolvedCriticalFailures(application: ReviewApplication): ReviewField[] {
  return (application.review?.fields || []).filter((field) => field.severity === "critical" && effectiveFieldStatus(field) === "FAIL");
}

export function criticalFields(application: ReviewApplication): ReviewField[] {
  return (application.review?.fields || []).filter((field) => field.severity === "critical" && effectiveFieldStatus(field) !== "PASS");
}
...
export function reviewerAiSummary(application: ReviewApplication): string {
...
export function matchesReviewerFilter(application: ReviewApplication, filter: ReviewerQueueFilter): boolean {
  ...
  if (filter === "new_submissions") return ["SUBMITTED", "READY_TO_SUBMIT"].includes(application.status);
  ...
  if (filter === "resubmitted") return application.status === "RESUBMITTED";
...
function fieldNeedsAttention(application: ReviewApplication, fieldKey: ReviewField["fieldKey"]): boolean {
  ...
  return ["FAIL", "NOT_FOUND", "NEEDS_REVIEW"].includes(effectiveFieldStatus(field));
}
