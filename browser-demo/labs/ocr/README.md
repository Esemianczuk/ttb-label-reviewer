...
- Current app runtime: Tesseract.js in the browser with fixed COLA-sheet crop regions.
- Lab candidates: EasyOCR, docTR, TrOCR, and Tesseract CLI.
- Future browser deployment path: PyTorch-trained models exported to ONNX, then run with ONNX Runtime Web or Transformers.js/WebGPU.
...
- `native-####`: original image downsampled so the long edge is at most that many pixels, for example `native-800`.
...
- `targeted-####`: run the targeted crop set from a downsampled source image, while mapping evidence boxes back to original-image coordinates.
