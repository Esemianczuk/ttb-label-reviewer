...
import { createApplicationPacketFromSample } from './application-packets.js';
...
export async function loadSamplePacket(packet) {
  ...
  const useFixture = packet.useFixtureByDefault !== false;

  const images = packet.images.map((image) => ({
      ...
      ocrResult: useFixture ? fixtureToOcrResult(fixture.images?.[image.fixtureKey || image.id], packet.title, image.name) : null,
      source: 'sample',
    }));
  const applicationPacket = createApplicationPacketFromSample(packet, expected, images);

  return {
    applicationPacket,
    expected: applicationPacket.expectedFields,
    images,
  };
}
